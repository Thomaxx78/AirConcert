...
function broadcast(code, msg, exclude = null) {
  ...
  clientMeta.forEach((meta, ws) => {
    if (meta.sessionCode === code && ws !== exclude && ws.readyState === WebSocket.OPEN)
      ws.send(data);
  });
}
...
wss.on('connection', ws => {
  ws.on('message', raw => {
    ...
    if (msg.type === 'join') {
      ...
      ws.send(JSON.stringify({ type: 'session', code: msg.code.toUpperCase(), players: [...session.players.values()], songId: session.songId }));
    ...
    if (msg.type === 'select_instrument') {
      ...
      broadcast(meta.sessionCode, { type: 'instrument_assigned', playerId: meta.playerId, instrument: p.instrument });
